...
// This is required because we're not able to guarantee that the shape of the
// input data conforms to the shape of the weights. In numpy, this is done
// automatically, while using nc::NdArray, calling `nc::dot` performs usual
// matrix multiplication while using the `*` operator performs broadcast
// multiplication.
auto dot_or_broadcast_mult = [](const nc::NdArray<Scalar> &A,
                                const nc::NdArray<Scalar> &B) {
...
perceptron::Layer::Layer(const int input_size,
...
perceptron::MultiLayerPerceptron::MultiLayerPerceptron(
        std::vector<Layer> layers,
        perceptron::random::Random<Scalar> &randomizer)
        : layers(std::move(layers)) {
    for (auto &layer: this->layers) {
        auto weight =
                nc::NdArray<Scalar>(nc::Shape(layer.output_size, layer.input_size));
        for (auto &w: weight)
            w = randomizer.random();
...
void perceptron::MultiLayerPerceptron::train(
        std::vector<std::pair<std::vector<Scalar>, std::vector<Scalar>>> &
        train_data,
        const perceptron::loss::Loss<Scalar> &loss,
        const uint32_t max_epochs,
        const double learning_rate,
        const std::optional<OnEpochHandler> on_epoch_handler) {
    for (auto epoch = 0; epoch < max_epochs; epoch++) {
        // Train the network by finding the gradient and update weights and biases
        // on each epoch
...
void perceptron::MultiLayerPerceptron::SGD(
        const std::vector<std::pair<std::vector<Scalar>, std::vector<Scalar>>> &
        train_data,
        const perceptron::loss::Loss<Scalar> &loss,
        const uint32_t max_epochs,
        const double learning_rate,
    ...
    for (auto epoch = 0; epoch < max_epochs; epoch++) {
        ...
        // Train the network with each batch (find the gradient and update weights
        // and biases)
...
void perceptron::MultiLayerPerceptron::gradient_descent(
    ...
    for (auto &[train, target]: batch) {
        ...
        std::transform(nabla_w.begin(), nabla_w.end(), nabla_w.begin(),
                       [&](const auto &w) {
                           i++;
                           return w + delta_w[i];
                       });
        i = -1;
        std::transform(nabla_b.begin(), nabla_b.end(), nabla_b.begin(),
                       [&](const auto &b) {
                           i++;
                           return b + delta_b[i];
                       });
    ...
    std::transform(
            nabla_w.begin(), nabla_w.end(), weights.begin(), [&](const auto &nw) {
                i++;
                return weights[i] -
                       (nw * (learning_rate / static_cast<double>(batch.size())));
            });
    i = -1;
    std::transform(
            nabla_b.begin(), nabla_b.end(), biases.begin(), [&](const auto &nb) {
                i++;
                return biases[i] -
                       (nb * (learning_rate / static_cast<double>(batch.size())));
            });
}
...
perceptron::MultiLayerPerceptron::backpropagate(
    ...
    std::vector<nc::NdArray<Scalar>> nabla_w(layers.size()),
            nabla_b(layers.size());
    ...
    // Calculate the partial derivative of the activation with respect to weighted
    // sum (for the last layer only).
    auto delta_a_last =
            (layers.end() - 1)->activation.differentiate(*(Z.end() - 1));
    ...
    *(nabla_w.end() - 1) =
            dot_or_broadcast_mult(partial_delta, (A.end() - 2)->transpose());
    *(nabla_b.end() - 1) = partial_delta;

    // After getting the derivative for the last part, we continue to find the
    // derivatives for the entire network. NOTE: There is some issues here where
    // if we have 3 or more layers, the dimensions for intermediate layers
    //       get mixed up (check later).
    for (int i = layers.size() - 2; i >= 0;
         i--) {  // NOLINT(*-narrowing-conversions)
        // Notice the partial delta from the last layer got updated.
        partial_delta =
                dot_or_broadcast_mult(weights[i + 1].transpose(), partial_delta) *
...
std::pair<std::vector<nc::NdArray<Scalar>>, std::vector<nc::NdArray<Scalar>>>
perceptron::MultiLayerPerceptron::feedforward(
        const nc::NdArray<Scalar> &input) {
    std::vector<nc::NdArray<Scalar>> Z(layers.size()), A(layers.size());
    // make the first activation as the train data because it is used as the input
    // to the first layer, it will be later override by the actual activation.
    A[0] = input.transpose();
...
nc::NdArray<Scalar> perceptron::MultiLayerPerceptron::predict(
        const nc::NdArray<Scalar> &input) {
    return feedforward(input).second.back();
}

perceptron::Evaluation perceptron::MultiLayerPerceptron::evaluate(
        std::vector<std::pair<std::vector<Scalar>, std::vector<Scalar>>> &_inputs,
        const perceptron::loss::Loss<Scalar> &loss) {
    auto predictions = std::vector<std::vector<Scalar>>();
    auto targets = std::vector<std::vector<Scalar>>();
    for (auto &[input, target]: _inputs) {
        predictions.push_back(this->predict(input).toStlVector());
    ...
    Evaluation evaluation{
            ...
            .targets = targets,
            .error = loss.apply(nc::NdArray(predictions), nc::NdArray(targets)),
            .accuracy = nc::count_nonzero(nc::equal(nc::NdArray(predictions), nc::NdArray(targets)))[0] /
